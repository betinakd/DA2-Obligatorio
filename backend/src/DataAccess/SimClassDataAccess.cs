...
public class SimClassDataAccess(SimulatorDbContext context) : ISimClassDataAccess
{
    ...
    public SimClass GetSimClassById(Guid id)
    {
        var simClass = _context.SimClasses
            ...
            .Include(c => c.Methods).ThenInclude(m => m.LocalVariables).ThenInclude(v => v.Instance)
            ...
            .Include(c => c.Methods).ThenInclude(m => m.Invocations).ThenInclude(i => i.Signature).ThenInclude(s => s.Parameters).ThenInclude(p => p.Instance)
            .Include(c => c.Methods).ThenInclude(m => m.Invocations).ThenInclude(i => i.Signature).ThenInclude(s => s.ReturnType)
    ...
    public bool HasCyclicDependency(Guid fromClassId, Guid? toClassId)
    {
        ...
        while(current != null)
        {
            ...
            current = current.BaseClass ?? _context.SimClasses
                .Include(c => c.BaseClass)
